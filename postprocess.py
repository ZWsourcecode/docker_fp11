...
for strHour in Hour_columns:
    ...
    darray_flexpart = xr.open_dataarray(ATT_PATH+"/"+floder_Flexpart+"/foot_nest")
    darray_flexpart = darray_flexpart.astype(np.float64)
...
Filename = "delta_14C_" + Station + "_" + simulate_date + ".csv"
delta_14C.to_csv(ATT_PATH+"/" + Filename, header=True,index=False, na_rep= "NaN")
# Filename = "delta_14C_" + Station + "_" + simulate_date + "_NPP.csv"
...
port = int(port) if port else 22  # Default SFTP port is 22 if not specified
...
remote_directory = '/data/CPrequests/icoscp/'+Station+"/" + str(Year)+"/"+str(Month).zfill(2)
