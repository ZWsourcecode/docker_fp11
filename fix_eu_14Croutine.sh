...
do
    ...
    bash flexpart11set_eu_14Croutine.sh $simulationidproject ${input}${year}/ $output $id $from $end $step $lon $lat $z $particles
    ...
    nohup $FLEXPARTPATH/FLEXPART_ETA pathnames >${output}${from:0:4}/${from}/log 2>&1 & 
    PID=$!
    ...
    python3 postprocess_eu.py $simulationid $(date -u -d "$from" +%Y-%m-%d)
